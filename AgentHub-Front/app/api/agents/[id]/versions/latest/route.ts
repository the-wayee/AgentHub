...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const url = `${base}/${encodeURIComponent(id)}/versions/latest`
  try {
    ...
    return new NextResponse(JSON.stringify({}), { headers: { "content-type": "application/json", "x-backend-url": url }, status: 502 })
  }
}
